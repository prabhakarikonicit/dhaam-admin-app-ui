...
const EditSubSubcategoryModal: React.FC<EditSubSubcategoryModalProps> = ({
  ...
  const [formData, setFormData] = useState<FormData>({
    name: currentSubSubcategoryItem?.name || "",
    description: currentSubSubcategoryItem?.description || "",
    // product: currentSubSubcategoryItem?.product || "",
    addMore: false,
  });

  useEffect(() => {
    if (currentSubSubcategoryItem) {
      setFormData({
        name: currentSubSubcategoryItem.name || "",
        description: currentSubSubcategoryItem.description || "",
        // product: currentSubSubcategoryItem.product || "",
  ...
  return (
    <CustomModal
      ...
      mode="edit"
      title={
        <span className="text-[15px] md:text-[16px] font-inter font-[600] text-black">
          Edit Sub - Subcategory in{" "}
          <span className="underline text-[16px] font-inter font-[600] text-black">
            {currentSubcategoryItem?.name}
          </span>
        </span>
      }
            ...
            <div className="bg-store-card p-4 rounded-custom border border-store-card ">
              <div className="mb-4">
                <label className="block mb-2">
                  <span className="text-paragraphBlack font-inter font-[500] text-[12px]">
                    Sub-subcategory Name <span className="text-danger">*</span>
                  </span>
                </label>
                <input
                  type="text"
              ...
              <div className="mb-4">
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-start w-full">
                  ...
                  <div className="flex items-center md:ml-4 sm:mt-6 sm:self-start ">
                    <input
                      type="checkbox"
                      id="addMoreSubSub"
                      name="addMore"
                      checked={formData.addMore}
                      onChange={handleChange}
