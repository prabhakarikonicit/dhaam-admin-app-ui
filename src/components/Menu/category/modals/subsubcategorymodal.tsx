...
const SubSubcategoryModal: React.FC<SubSubcategoryModalProps> = ({
  ...
  const [formData, setFormData] = useState<FormData>({
    name: "Coffee", // Auto-filled as required
    description: "",
    product: "",
    addMore: false,
  });
  ...
  const handleSave = () => {
    ...
    if (formData.addMore) {
      ...
      // Close the modal if not adding more
      onClose();
    }
  };
  ...
  return (
    <CustomModal
      ...
      title={
        <span className="text-[13px] md:text-[16px] font-inter font-[600] text-black">
        Add New Sub - Subcategory in{" "}
          <span className="underline text-[13px] font-inter font-[600] text-black">{currentSubcategoryItem?.name}</span>
        </span>
      }
      ...
      confirmText="Save"
      customFooter={
        <div className="w-full max-w-5xl mx-auto mt-[-15px] mb-8">
          <div>
